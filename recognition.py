...
# Model based on angles and sides
x_train, x_test, y_train, y_test = train_test_split(features, names_encoded, test_size=0.2, random_state=42)
y_train = keras.utils.to_categorical(y_train, num_classes)
y_test = keras.utils.to_categorical(y_test, num_classes)

x_train_i, x_test_i, y_train_i, y_test_i = train_test_split(images, names_encoded, test_size=0.2, random_state=42)
...
model.compile(loss=keras.losses.categorical_crossentropy,
              ...
              metrics=['accuracy'])
model.fit(x_train, y_train, batch_size=32, epochs=2000, verbose=1, validation_data=(x_test, y_test))

# Just Images

model = Sequential()
model.add(Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=input_shape_i))
